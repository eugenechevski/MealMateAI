  ...
  useEffect(() => {
    async function fetchCuisines() {
      ...
      if (data && !error) {
        ...
        let uniqCuisines: { [key: string]: string } = {};
        for (let i = 0; i < data.length; i++) {
          if (data[i]?.cuisine !== null) {
            uniqCuisines[data[i].cuisine as string] = "";
          }
        }
        setCuisines(Object.keys(uniqCuisines));
      }
    }
    ...
    >
      ...
      <div className="primary-form-field">
        ...
        <Select {...register("cuisine", { required: true })}>
          {cuisines.map((cui) => (
            <SelectItem key={cui}>{cui}</SelectItem>
          ))}
        </Select>
      ...
      <div className="primary-form-field">
        ...
        {errors.caloriesPerServing && (
          <span>This field is required and must be a non-negative number</span>
        )}
      </div>
